<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-6 gap-4">
        @foreach([
            'eligible' => 'Eligible',
            'needs_binding' => 'Needs Matomo',
            'bound_attention' => 'Needs Attention',
            'covered' => 'Covered',
            'excluded' => 'Excluded',
            'unmapped' => 'Unmapped Sites',
        ] as $key => $label)
            <div class="rounded-lg bg-white dark:bg-gray-800 shadow-sm p-4">
                ...
                <div class="mt-2 text-2xl font-semibold text-gray-900 dark:text-gray-100">{{ $stats[$key] }}</div>
            </div>
        @endforeach
    </div>
    ...
    <div class="grid grid-cols-1 gap-6">
        ...
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6">
                ...
                <div class="mt-4 space-y-3">
                    @forelse($boundAttention as $item)
                        @php($audit = $item['matomo_source']?->latestInstallAudit)
        ...
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6">
                ...
                <div class="mt-4 space-y-3">
                    @forelse($unmappedObservations as $item)
                        @php($observation = $item['observation'])
